...
{  
  ...
  if(gcmd_line.lisp_file_name_flag == 1)
  {
    printf("\n  write to lisp output file: %s \n", gcmd_line.lisp_file_name);

    FILE *file;
    file = fopen(gcmd_line.lisp_file_name,"w");
    fprintf(file,"(");
    fprintf(file,"(result ok)");
    fprintf(file,"(ops %d)",gnum_plan_ops);
    ...
    for ( i = 0; i < gnum_plan_ops; i++ )
    {
      ...
      if ( !a->norm_operator && !a->pseudo_action ) {
        ...
        for ( j = 0; j < a->num_name_vars; j++ ) {
          fprintf(file," %s", gconstants[a->name_inst_table[j]]);
        }
      }
      fprintf(file,")");
    }
