...
/*********************************************************************
 *
 * minor modifications by Andreas Steck (steckqhs-ulm.de) to support
 * Lisp-file ouput of the plan.
 *
 *********************************************************************/ 
...

{
  ...
  printf("\nusage of metric-ff:\n");
  printf("Modified by Andreas Steck (steck@hs-ulm.de) to support LISP file output. (2010)\n");

  printf("\nOPTIONS   DESCRIPTIONS\n\n");
  printf("-p <str>    path for operator and fact file\n");
  printf("-o <str>    operator file name (domain)\n");
  printf("-f <str>    fact file name\n");
  printf("-l <str>    LISP output file name\n\n");
...

{
  ...
  memset(gcmd_line.lisp_file_name, 0, MAX_LENGTH);
  gcmd_line.lisp_file_name_flag = 0;
  ...
  while ( --argc && ++argv ) {
	...
	case 'f':
	  ...
	  break;
        case 'l':
          strncpy( gcmd_line.lisp_file_name, *argv, MAX_LENGTH );
          gcmd_line.lisp_file_name_flag = 1;
          FILE *file;
          file = fopen(gcmd_line.lisp_file_name,"w");
          fprintf(file,"((result error))");
          fclose(file); 
          break;
	case 'i':
